...
class WorkflowContextEngine:
    ...
    def __init__(self, workflow_engine: WorkflowEngine = None, scheduler: Scheduler = None):
    ...
    def get_current_context(self, refresh: bool = False) -> ContextSnapshot:
        ...
        if not refresh and self._context_cache and self._cache_expiry and now < self._cache_expiry:
    ...
    def _generate_context(self) -> ContextSnapshot:
        ...
        priority_insights = self._generate_priority_insights(active_workflows, scheduled_tasks)
    ...
    def _get_current_workflow(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            current = None
            for workflow in sorted(workflows, key=lambda w: w.get('updated_at', ''), reverse=True):
                if workflow.get('status') == WorkflowStatus.IN_PROGRESS.value:
                    current = workflow
                    break
                elif workflow.get('status') == WorkflowStatus.ACTIVE.value and not current:
                    current = workflow
                elif workflow.get('status') == WorkflowStatus.PENDING.value and not current:
                    current = workflow
    ...
    def _get_active_workflows(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return sorted(active_workflows, key=lambda w: (
    ...
    def _get_recent_activity(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for workflow in workflows[-10:]:  # Last 10 workflows
                activities.append({
                    'timestamp': workflow.get('updated_at', workflow.get('created_at')),
                    'type': 'workflow_updated',
                    'description': f"Updated workflow: {workflow.get('title')}",
                    'workflow_id': workflow.get('id')
                })

            return sorted(activities, key=lambda a: a.get('timestamp', ''), reverse=True)[:5]

        except Exception as e:
            return []

    def _determine_user_focus(self, current_workflow: Optional[Dict], active_workflows: List[Dict]) -> Optional[str]:
        """Determine what the user should focus on"""
        if current_workflow and current_workflow.get('status') == WorkflowStatus.IN_PROGRESS.value:
        ...
        due_soon = [w for w in active_workflows if w.get('due_date', '9999-12-31') <= today]
        if due_soon:
            return f"Due today: {due_soon[0].get('title')}"

        return None

    def _generate_priority_insights(self, workflows: List[Dict], tasks: List[Dict]) -> Dict[str, Any]:
        """Generate insights about priorities and recommendations"""
        insights = {
            'total_active_workflows': len(workflows),
            'high_priority_count': len([w for w in workflows if w.get('priority', 3) <= 2]),
            'overdue_count': 0,
            'recommendations': []
        }
        ...
        if insights['overdue_count'] > 0:
            insights['recommendations'].append("You have overdue workflows that need attention")

        if insights['high_priority_count'] > 3:
            insights['recommendations'].append("Consider focusing on high-priority items to reduce workload")

        if len(workflows) > 10:
            insights['recommendations'].append("You have many active workflows. Consider consolidating or completing some")
    ...
    def _save_context(self, context: ContextSnapshot):
        ...
        try:
            context_file = self.context_dir / f"context_{context.timestamp.replace(':', '-')}.json"
    ...
    def get_ai_optimized_context(self, provider_name: str = "default") -> Dict[str, Any]:
